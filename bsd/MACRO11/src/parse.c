    ...
    unsigned *flt)
{
...

    {
        ...
        for (part = 0; part < 4; part++) {
            bits[part] = (unsigned) scaled & 0xffff;
            scaled -= (unsigned) scaled;
            if (part < 3)
                scaled *= 65536.0;
        }
    }

    /* FLT2/^F store the FLT4 fraction shifted right one bit, with the
       exponent reduced by one. Round to the nearest FLT2 value first. */
    if (size < 4) {
        unsigned        sum;
        int             part;
        ...
        bits[2] = sum & 0xffff;
        sum >>= 16;
        for (part = 1; part >= 0 && sum != 0; part--) {
            sum += bits[part];
            bits[part] = sum & 0xffff;
            sum >>= 16;
        }

        bits[3] = (bits[3] >> 1) | ((bits[2] & 1) << 15);
        bits[2] = (bits[2] >> 1) | ((bits[1] & 1) << 15);
        bits[1] = (bits[1] >> 1) | ((bits[0] & 1) << 15);
        bits[0] >>= 1;
        exp--;
    }

    flt[0] = (unsigned) (sign | (exp << 7) | (bits[0] & 0x7F));
    if (size > 1) {
        flt[1] = bits[1];
        if (size > 2) {
            flt[2] = bits[2];
